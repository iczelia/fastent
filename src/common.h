...
#define FASTENT_PRINTF(f, a) __attribute__((__format__(__printf__, f, a)))
...
void fastent_message(const char * fmt, ...) FASTENT_PRINTF(1, 2);
...
/*  SWAR per-byte bit reverse of a u64 (8 lanes, 3 mask-swap steps),
    bit-identical to fastent_bitrev8_ per byte.  */
