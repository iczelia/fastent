/*  fastent: AArch64 NEON batched Berlekamp-Massey (2 windows/pass).
    AArch64 only: the lane logic needs 64-bit integer compare/shift;
    ARMv7-A NEON lacks them, so it stays on the scalar reference.
...
/*  Two 512-bit windows run in the 2 u64 lanes of one uint64x2_t; the
