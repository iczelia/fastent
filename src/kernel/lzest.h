...
/*  Start each 4 MiB block at an absolute file offset with a fresh table.
    Integer counts can be merged in any order.  */
...
/*  Optional LZ77F result tables, allocated separately from fastent_result.  */
...
/*  Buffer and parse one grid block at a time with a fresh match table.
    Periodically add the u16 offset counts in off_blk to off_par.  */
typedef struct {
...
/*  Initialise at an absolute offset.  Feed splits input at grid boundaries;
    flush parses the tail, and merge adds counts.  Feed and flush return
...
struct fastent_result;  /*  Forward declaration: analyze.h includes this header.  */

/*  Allocate zeroed result tables; return NULL on allocation failure.  */
struct fastent_lz77f_tables * fastent_lz77f_tables_alloc(void);
void fastent_lz77f_tables_free(struct fastent_lz77f_tables * t);

/*  Fill the scalar results and, if out->lz is set, the distribution tables.
    n is the total byte count.  Handle fewer than two matches explicitly
