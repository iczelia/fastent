...
/*  SVE2 -ee level-2 digram+run kernel, bit-identical to scalar for any -j and
    VL: never folds, key=(dg_prev<<8)|cur, plane=abs-index parity, run via
    CTZ64 = fastent_lr_one.  */
...
void digram_bytes_sve2(
  ...
  /*  Scalar prologue: the SVE window needs buf[base-1], so starts at index >=
      1.  */
  sz k = i0;
  if (k == 0) {
    u32 c = buf[0];
  ...
  ALIGN(64) u8  neq[SVE2_DG_MAXVL];
  ALIGN(64) u16 keys[SVE2_DG_MAXVL];
  ...
  while (k >= 1 && (u64) k + W <= (u64) len) {
    ...
    /*  Scalar inc-mem scatter from the staged keys with one-ahead prefetch
        and absolute-index parity round-robin.  */
    {
      const u16 * RESTRICT sp = keys;
      __asm__("" : "+r"(sp) :: "memory");
      u32 * RESTRICT t0 = t;
