...
if (!exists("host"))     host     = 'Ryzen 9 5900X, 12C/24T, dual-channel DDR4'
...
medline(m, l) = sprintf("< awk -v M=%s -v L=%s '$2==M&&$3==L&&$4~/^[0-9]+$/{n[$4]++;A[$4,n[$4]]=$9} END{c=\"1 2 4 8 16 24\";split(c,J,\" \");for(i=1;i<=6;i++){j=J[i];g=n[j];for(p=1;p<=g;p++)t[p]=A[j,p];for(a=2;a<=g;a++){x=t[a];b=a-1;while(b>=1&&t[b]>x){t[b+1]=t[b];b--}t[b+1]=x}if(g)print j,(g%%2)?t[(g+1)/2]:(t[g/2]+t[g/2+1])/2}}' %s", m, l, data)
...
set ytics (50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000)
set yrange [48:30000]
...
set arrow from 1,eb_byte to 24,eb_byte nohead dt 2 lw 1 lc rgb cbold("byte")
set arrow from 1,eb_bit  to 24,eb_bit  nohead dt 2 lw 1 lc rgb cbold("bit")
set arrow from 1,eb_fold to 24,eb_fold nohead dt 2 lw 1 lc rgb cbold("byte-fold")
